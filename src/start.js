...
export const Start = {
  start() {
    ...
    const startServer = (dir, name) => {
      return new Promise((resolve, reject) => {
        ...
        if (!fs.existsSync(fullPath)) {
          console.log(
            `⚠️  [MrServer CLI] Directory ${fullPath} does not exist.`,
          );
          resolve();
          return;
        }
    ...
    Promise.all([startServer("api", "Backend"), startServer("ui", "Frontend")])
      .then(() => {
        console.log("✅ [MrServer CLI] MrServer started!");
        console.log(
          "🌐 [MrServer CLI] Access it at http://127.0.0.1:1101 or the device's IP address",
        );
      })
      .catch((err) => {
